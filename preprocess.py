...
from onmt.inputters.dataset_base import SigmorphonDataset, lang_name, SigmorphonDatasetBPE
...
def main():
    ...
    fields = get_fields(
        ...
        inflection=opt.inflection_field, use_bpe = opt.use_bpe)
    ...
    if opt.use_bpe:
        train_dataset = SigmorphonDatasetBPE(
        fields, opt.train, opt.bpe_train, lang_src=opt.lang_src,
    ...
    if opt.use_bpe:
        valid_dataset = SigmorphonDatasetBPE(fields, opt.valid, 
            opt.bpe_valid, lang_src=opt.lang_src)
    else:
