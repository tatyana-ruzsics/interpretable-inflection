...
def global_attn_support(attn_type, n_words_global, **kwargs):
    return kwargs.get(attn_type + "_attended", 0) / n_words_global
...
class Trainer(object):
    ...
    def __init__(self, model, model_opt, fields, train_loss, valid_loss, optim,
        ...
        if self.log_global_gate_attn:
            #for attn_type in ["gate_lemma_global", "gate_infl_global"]:
            attn_types = ["gate_lemma_global_"+str(k) for k in range(self.model_opt.global_gate_heads_number)]
            for attn_type in attn_types:
                support_func = partial(global_attn_support, attn_type)
                self.metrics.append(support_func)
                self.templates.append(attn_type + " global attn: {:6.2f}")
                self.metric_names.append(attn_type + "_global_attn_support")
        # if self.log_global_gate_attn_mix:
        #     #for attn_type in ["gate_lemma_global", "gate_infl_global"]:
        #     attn_types = []
        #     for k in range(self.model_opt.global_gate_heads_number):
        #         attn_types.append("gate_lemma_subw_global_"+str(k))
        #         attn_types.append("gate_lemma_char_global_"+str(k))
        #     for attn_type in attn_types:
        #         support_func = partial(global_attn_support, attn_type)
        #         self.metrics.append(support_func)
        #         self.templates.append(attn_type + " global attn: {:6.2f}")
    ...
    # @property
    # def log_global_gate_attn_mix(self):
    #     return self.model_opt.global_gate_heads_mix == True

    @property
    def log_global_gate_attn(self):
        return self.model_opt.global_gate_heads == True

    @property
    def log_sparse_attn(self):
        return self.model_opt.global_attention_function == "sparsemax" or self.model_opt.global_attention_function == "fusedmax"
        ...
        for j in range(0, target_size - 1, trunc_size):
            ...
            if self.log_global_gate_attn:
                for k in range(self.model_opt.global_gate_heads_number):
                    ...
                    bottled_attn = v.view(v.size(0) * v.size(1), -1)
                    attn_support = bottled_attn.gt(0).sum(dim=1)
                    batch_stats["n_words_global"] += tgt.size()[1]
                    n_attended = attn_support
                    batch_stats[k + "_attended"] += n_attended.sum().item()
            ...
            #             attn_supp = bottled_attn.gt(0).sum(dim=1)
            #             n_attended = attn_supp
            #             batch_stats["n_words_global"] += tgt.size()[1] # batch size
            #             batch_stats[k + "_attended"] += n_attended.sum().item()
            if self.log_gate:
                seq_len, batch_size, classes = attns["gate"].size()
                bottled_gate = attns["gate"].view(seq_len * batch_size, -1)
    ...
    def _report_step(self, step, stats, label, epoch_start):
        ...
        #     for attn_type in attn_types:    
        #         support_func = partial(global_attn_support, attn_type)
        #         logger.info("{} {} global attn: {:6.2f}".format(
