...
# can be deleted
# class InputFeedRNNDecoderGlobalHead(RNNDecoderBase):

#     def __init__(self, emb, rnn, attn, global_head, dropout=0.0):
#         super(InputFeedRNNDecoderGlobalHead, self).__init__(emb, rnn, attn, dropout=0.0)
...
#         for i,emb_t in enumerate(tgt_emb.split(1)):
#             emb_t = emb_t.squeeze(0)
#             input_feed = input_feeds[-1]
#             decoder_input = torch.cat([emb_t, input_feed], 1)
#             rnn_output, dec_state = self.rnn(decoder_input, dec_state)
...
#             if i==0:
#                 globalattn_memory_bank, glbal_gate_attns = self.global_head(
#                     rnn_output.unsqueeze(1), memory_bank, **kwargs)

#             decoder_output, p_attn = self.attn(
#                 rnn_output.unsqueeze(1), memory_bank, globalattn_memory_bank, **kwargs)
#             decoder_output = decoder_output.squeeze(0)
#             # p_attn used to be a tensor. Now it is a dictionary with string
#             # keys and tensor values.
#             for k, v in p_attn.items():
...
#         # add global gate attention
#         for k, v in glbal_gate_attns.items():
#             attns[k].append(v.squeeze(0))
