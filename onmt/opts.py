...
def model_opts(parser):
    ...
    # group.add("--fused_msd", "-fused_msd", action="store_true",
    #           help="""Encode inflection sequence with a bilstm plus one dimesion for average.""")
    ...
    group.add('--global_attention_function', '-global_attention_function',
              type=str, default="softmax", choices=["softmax", "sparsemax", "fusedmax"])
    group.add("--inflection_attention", "-inflection_attention",
              action="store_true", help="""Second attention mechanism over
              inflection tags.""")
    group.add('--inflection_attention_function', '-inflection_attention_function',
              type=str, default=None, choices=["softmax", "sparsemax", "fusedmax"],
              help="""By default, same as global_attention_function.""")
    ...
    # group.add("--separate_heads", "-separate_heads", action="store_true",
    #           help="""If using an attention gate with separate outputs, whether to use separate
    #           lemma and inflection attention heads for gate and state vectors.""")
    group.add("--global_gate_heads", "-global_gate_heads",
              action="store_true", help="""If using an attention gate, whether to use separate
              output layers for the lemma and inflection attention heads, and
              use separate lemma and inflection attention heads for gate and state vectors:
              global two-head unshared attention for gate, two-head unshared attention for state vectors.""")
    # group.add("--global_gate_heads_mix", "-global_gate_heads_mix",
    #           action="store_true", help="""If using an attention gate, use global attention
    #           heads over lemma characters and lemma subwords.""")
    group.add("--global_gate_heads_number", "-global_gate_heads_number", default=1,
              type=int, help="Number of global gate heads")
    group.add("--global_gate_head_nonlin", "-global_gate_head_nonlin", action="store_true",
              help="""If using global_gate_heads, whether to use non-linear transform in global gate attention.""")
    group.add("--global_gate_head_combine", "-global_gate_head_combine", action="store_true",
              help="""If using global_gate_heads, whether to combine global heads with local heads.""")
...
def preprocess_opts(parser):
    ...
    group.add("--use_bpe", "-use_bpe", action="store_true",
              help="""Use bpe splits for global gate attention.""")
    group.add("--bpe_train", "-bpe_train",
              help="""Path to sigmorphon bpe training data.""")
    group.add("--bpe_valid", "-bpe_valid",
              help="""Path to sigmorphon bpe valid data.""")
...
def translate_opts(parser):
    ...
    group.add('--bpe_file', '-bpe_file',
              help="Paths to SIGMORPHON bpe test files")
    group.add("--use_bpe", "-use_bpe", action="store_true",
              help="""Use bpe splits for global gate attention.""")
