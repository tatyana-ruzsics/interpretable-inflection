...
from onmt.inputters.dataset_base import SigmorphonDataset, SigmorphonDatasetBPE
...
class Translator(object):
    ...
    def __init__(self, model, fields, opt, out_file=None):
        ...
        self.bpe_file = opt.bpe_file
        self.use_bpe = opt.use_bpe
    ...
    def translate(self, corpora):
        if self.use_bpe:
            data = SigmorphonDatasetBPE(self.fields.copy(), corpora, self.bpe_file)
        else:
    ...
    def translate_batch(self, batch):
        ...
        globalattn_memory_bank = encoder_out.get("globalattn_memory_bank", None)
        global_gate_attns = encoder_out.get('global_gate_attns', None)

        self.model.init_decoder_state(enc_states)

        if globalattn_memory_bank is not None:
            #print(globalattn_memory_bank.size())
            #print(tile(globalattn_memory_bank, beam_size, dim=1).size())
            #print(tile(globalattn_memory_bank, beam_size).size())
            #side_info["globalattn_memory_bank"] = tile(globalattn_memory_bank, beam_size, dim=1)
            side_info["globalattn_memory_bank"] = globalattn_memory_bank
            #b.record_ggh(global_gate_attns)
        ...
        if globalattn_memory_bank is not None:
            #side_info["globalattn_memory_bank"] = tile(globalattn_memory_bank, beam_size, dim=1)
            #side_info["globalattn_memory_bank"] = globalattn_memory_bank
            side_info["globalattn_memory_bank"] = tile(globalattn_memory_bank, beam_size)
            b.record_ggh(global_gate_attns)
        ...
        for i in range(self.max_length):
            ...
            # print('before iter ', i)
            # for label, label_attn in b.attn.items():
            #     print(label)
            #     print(len(label_attn))
            #     print(label_attn[0].size())
            ...
            #print('dec_attn.keys',list(dec_attn.keys()))
        ...
        if self.attn_path is not None:
            ...
            #except:
            #    print(self.itos(src, "src"))
            #    print(scores, ks)
            #    #print(self.itos(hyps[0], "tgt"))
            #    print(attn)
