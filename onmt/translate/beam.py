...
class Beam(object):
    ...
    def record_ggh(self, global_gate_attns):
        for k, v in global_gate_attns.items():
            self.g_attn[k] = v.squeeze(0)
    ...
    def advance(self, word_probs, attn_out):
        ...
        * `attn_out`- dictionary of attentions at the last step
        ...
        if self.current_state[0] == self._eos:
            #print('prev final', self.prev_ks)
            #print('next final', self.next_ys)
            self.eos_top = True
    ...
    def sort_finished(self):
        if self.n_best >= 1: # changed > to >= to fix the cases where no hypo was finished but the beam hitted max_length iteration
            i = 0
    ...
    def get_hyp(self, timestep, k):
        ...
        out_probs = []
        # print(timestep, k)
        # print(range(len(self.prev_ks[:timestep]) - 1, -1, -1))
        ...
        for label,label_attn in self.g_attn.items():
            attn[label] = label_attn

        # for label, label_attn in attn.items():
        #         print(label)
        #         print(label_attn.size())
                #print(len(label_attn))
                #print(label_attn[0].size())
